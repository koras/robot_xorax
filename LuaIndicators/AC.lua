...
if (SP>0) and (LP>0) then
	...
	local ao = AC_AO(I, {SHORT_Period=SP, LONG_Period=LP, Metod = M, VType=VT}, ds)
	if CandleExist(I,ds) then
		if I~=it.p then it={p=I, l=it.l+1} end
		if it.l >= LP and ao then
			local Out = AOds_MA(it.l-LP+1, {Period=SP, Metod = M, VType=ANY}, {[it.l-LP+1] = ao})
			if Out then Out=ao-Out end
			return Out 
		end
	end
end
...
function AO() --Awesome Oscillator ("AO")
	local S_MA = MA()
	local L_MA = MA()
	local it = {p=0, l=0}
return function (I, Fsettings, ds)
...
if (SP>0) and (LP>0) then
	...
	local S = S_MA(I, {Period=SP, Metod = M, VType=VT}, ds)
	local L = L_MA(I, {Period=LP, Metod = M, VType=VT}, ds)
	if CandleExist(I,ds) then
		if I~=it.p then it={p=I, l=it.l+1} end
		if it.l >= LP and S and L then
			return S - L
		end
	end
end
