...
function CV() --Chaikin’s Volatility ("CV")
	local CV_MA=MA()
	local rCV_MA={}
	local it = {p=0, l=0}
...
if (P>0) then
	...
	local t_ma = CV_MA(I, {Period=P, Metod=M, VType=DIFFERENCE}, ds)
	if CandleExist(I,ds) then
		if I~=it.p then it={p=I, l=it.l+1} end
		local Ip,Ippp = Squeeze(it.l,P),Squeeze(it.l-P,P)
		rCV_MA[Ip]=t_ma
		if it.l>=P*2 then
			Out = (rCV_MA[Ip] - rCV_MA[Ippp]) * 100 / rCV_MA[Ippp]
			return Out
		end
	end
end
