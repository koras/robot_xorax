...
line = {{
		...
		{
		Name = "ENVELOPES - upper",
		Type = TYPE_LINE, 
		Color = RGB(0, 206, 0)
		},
		{
		Name = "ENVELOPES - lower",
		Type = TYPE_LINE, 
		Color = RGB(0, 162, 232)
		}
		},
...
function ENVELOPES() --Envelopes ("ENVELOPES")
	local Envelopes_MA=MA()
	local it = {p=0, l=0}
return function (I, Fsettings, ds)
local Fsettings=(Fsettings or {})
local P = (Fsettings.Period or 20)
local k = (Fsettings.Coefficient or 2)
...
if (P>0) then
	...
	local t_ma = Envelopes_MA(I, {Period=P, Metod = M, VType=VT}, ds)
	if CandleExist(I,ds) then
		if I~=it.p then it={p=I, l=it.l+1} end
		if it.l >= P and t_ma then 
			return t_ma, t_ma*(1+k/100), t_ma*(1-k/100)
		end
	end
end
return nil,nil,nil
