Settings = {
Name = "*SROC (Smoothed Rate Of Change)", 
Period = 10, 
...
function SROC() --Smoothed Rate Of Change ("SROC")
	local SROC_MA=MA()
	local tmp = {}
	local it = {p=0, l=0}
...
if (P>0) then
	...
	local t_ma = SROC_MA(I, {Period=P, Metod = M, VType=VT}, ds)
	if CandleExist(I,ds) then
		if I~=it.p then it={p=I, l=it.l+1} end
		tmp[Squeeze(it.l,S)] = t_ma
		if it.l >= P + S then
			return tmp[Squeeze(it.l,S)]/tmp[Squeeze(it.l-S,S)] * 100
		end
	end
end
return nil
