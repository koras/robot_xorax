...
VType = "Volume", --Open, High, Low, Close, Volume, Median, Typical, Weighted, Difference
Percent=1,
line = {{
...
function VO() --"Volume" Oscillator ("VO")
	local S_MA = MA()
	local L_MA = MA()
	local it = {p=0, l=0}
...
local VT = (Fsettings.VType or VOLUME)
local Percent = (Fsettings.Percent or 1)
if (SP>0) and (LP>0) then
	if I == 1 then 
		it = {p=0, l=0}
	end
	local So = S_MA(I, {Period=SP, Metod = M, VType=VT}, ds) 
	local Lo = L_MA(I, {Period=LP, Metod = M, VType=VT}, ds)
	if CandleExist(I,ds) then
		if I~=it.p then it={p=I, l=it.l+1} end
		if (it.l-math.max(SP, LP)+1 > 0) then
			if Percent==0 then
				return (So-Lo)
			else
				return 100*(So-Lo) / Lo
