...
LIPS_Period = 5, 
LIPS_Shift = 3,
Metod = "SMMA", --(SMA, MMA, EMA, WMA, SMMA, VMA)
...
function ALLIGATOR() --Alligator ("ALLIGATOR")
	local LIPS = {}
	local TEETH = {}
	local JAW = {}
	local LIPS_MA = MA()
...
local pJ = (Fsettings.JAW_Period or 13)
local sJ = (Fsettings.JAW_Shift or 8)
local pT = (Fsettings.TEETH_Period or 8)
local sT = (Fsettings.TEETH_Shift or 5)
local pL = (Fsettings.LIPS_Period or 5)
local sL = (Fsettings.LIPS_Shift or 3)
local M = (Fsettings.Metod or SMMA)
local VT = (Fsettings.VType or MEDIAN)
if (pJ>0) and (pT>0) and (pL>0) then
	if I == 1 then
		LIPS = {}
		TEETH = {}
		JAW = {}
	...
	LIPS[Squeeze(it.l,pL)] = LIPS_MA(I, {Period=pL, Metod = M, VType=VT}, ds)
	TEETH[Squeeze(it.l,pT)] = TEETH_MA(I, {Period=pT, Metod = M, VType=VT}, ds)
	JAW[Squeeze(it.l,pJ)] = JAW_MA(I, {Period=pJ, Metod = M, VType=VT}, ds)
	return JAW[Squeeze(it.l-sJ,pJ)], TEETH[Squeeze(it.l-sT,pT)], LIPS[Squeeze(it.l-sL,pL)]
end
return nil,nil,nil
end
end
