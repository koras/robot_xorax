...
function wSignal(_event)
	arr = {
		...
		[30] = 'Сработал стоп на контракты',   
		[31] = 'Сработал стоп в режиме эмуляции',   
		[32] = 'Сработал стоп',   
		[33] = 'снимаем заявки которые установлены на верху, в зависимости от количества заявок',
		[34] = 'сняли заявку после срабатывания стопа',
		[35] = 'отравляем транкзакцию на стоп заявку',
		[36] = 'снимаем все стопы',
		[37] = 'обновляем стопы',
		[38] = 'Покупка запрещена, низкий диапазон покупок',   
...
function word(key)
	...
	arr = {	  
		...
		['not_buy_low'] = "Низ коридора покупок",

		 
		['count_calculate_candle'] = "Количество свечей (вверх/низ)",
		['not_found_tag'] = tostring("В своиство графика необходимо добавить " .. 
		setting.tag ..".  Откройте график и в области Price перейдите в вкладку дополнительно, в самом низу есть поле 'Идентификатор', туда и добавьте " .. 
		setting.tag ..". После нажмите Применить и перезапустите робота") 
	}
