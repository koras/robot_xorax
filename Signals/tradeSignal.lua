...
local function calculateVolume(volume)
    ...
    -- новая свеча
    if (setting.old_number_of_candles + count_of_candle ~=
        setting.number_of_candles) then
    ...
    if oldVolume == volume then return end
...
local function getSignal(tg, callback)
    ...
    while i >= 1 do
        ...
        if bars_temp[j - 1].datetime.hour >= 10 then
            sk = true
            if bars_temp[j - 1].datetime.hour == 18 and
                bars_temp[j - 1].datetime.min == 45 then sk = false end
            if sk then
